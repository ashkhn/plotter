...
ebmpBYTE& BMP::redPixel(int rx, int cx) {
  return this->operator()(rx, cx)->Red;
}

ebmpBYTE& BMP::greenPixel(int rx, int cx) {
  return this->operator()(rx, cx)->Green;
}

ebmpBYTE& BMP::bluePixel(int rx, int cx) {
  return this->operator()(rx, cx)->Blue;
}
 ...
 *InputImage(NewWidth-1,NewHeight-1) = *OldImage(OldWidth-1,OldHeight-1);
 return true;
      ...
      // double tempDouble = 0.30*( Image(i,j)->Red   ) +
